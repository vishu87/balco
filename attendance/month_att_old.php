...
<?php
	...
	<?php
		...
		$sql_case="SELECT id, father_mob, name, dos, second_group from students WHERE train_city='$city' AND center='$center' AND groupid='$group' AND active='0'";
	...
	<div id="atten" align="center">
		...
		while($row_att = mysql_fetch_array($result_att))
		{
			...
			$result_check = mysql_query(" SELECT DISTINCT groupid from attendance WHERE student_id='$row_att[id]' AND month='$month' AND year='$year' ");

			$row_group_names= mysql_fetch_array($result_check);
			$total_groups = mysql_num_rows($result_check);
			
			if( ($total_groups == 1 && $row_att["second_group"] != $row_group_names["groupid"] ) && $row_group_names["groupid"] != $groupid)
			{
				...
				$query_group_det = mysql_query("SELECT group_name, center_name from groups where id='$row_group_names[groupid]' limit 1");
				$row_group_det = mysql_fetch_array($query_group_det);
				
				echo '<td style="background:#EEE;" colspan="31">ATTENDANCE IN '.$row_group_det["group_name"].', '.$row_group_det["center_name"].'</td>';
				echo '<td style="background:#EEE;"></td><td style="background:#CCC;"></td></tr>';
				continue;
			}
			...
			$sql_st_at="SELECT date,attendance,ds from attendance WHERE student_id='$row_att[id]' AND month='$month' AND year='$year' ";
