...
if($count_form == 1){
	...
	for($i=1;$i<=31;$i++) {
		if($_POST["cl".$i] == 1) {	
			mysql_query("INSERT INTO attendance (student_id, date, month, year, groupid,ds) VALUES ('dm', '$i','$month', '$year', '$groupid','1')");
		}
	}
}
...
foreach ($_POST["student_ids"] as $id) {
	...
	for($i=1;$i<=31;$i++) {
		if($_POST["cl".$i] == 1) {
			...
			mysql_query("INSERT INTO attendance (student_id, date, month, year, groupid, attendance,ds) VALUES ('$id', '$i','$month', '$year', '$groupid', '$pre','1')");
